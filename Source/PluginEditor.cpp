...
void SpectroscopeAudioProcessorEditor::resized()
{
    ...
    // Waveform takes the top third, spectrogram the rest. Each gets a
    ...
    spectrogramLabelArea = area.removeFromTop (labelHeight);
    spectrogramView.setBounds (area.reduced (screenInset, 0));
}
                                                            ...
                                                            juce::StringRef annotation)
{
    ...
    Theme::drawCornerBrackets (g, screen.expanded (6), Theme::amberDim.withAlpha (0.75f), 12, 1.4f);

    auto label = labelArea.reduced (10, 0);

    g.setColour (Theme::bone.withAlpha (0.85f));
    g.setFont (Theme::mono (9.5f, true));
    g.drawText (Theme::spaced (caption), label.removeFromLeft (label.getWidth() / 2),
                juce::Justification::centredLeft);

    g.setColour (Theme::boneDim.withAlpha (0.65f));
...
void SpectroscopeAudioProcessorEditor::paintHeader (juce::Graphics& g)
{
    ...
    g.setColour (Theme::boneDim.withAlpha (0.7f));
    ...
    g.setColour (Theme::amberBright.withAlpha (0.9f));
    ...
    // Lamp cluster between the two.
    const juce::String lampNames[] = { "PWR", "SIG", "ERR" };
    const juce::Colour lampColours[] = { Theme::amber, Theme::phosphor, Theme::rust };
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        g.setColour (lampStates[i] ? Theme::bone.withAlpha (0.8f) : Theme::boneDim.withAlpha (0.4f));
...
{
    // Warm chassis with a slight gradient, as though lit from above.
    g.setGradientFill (juce::ColourGradient (Theme::shellMid, 0.0f, 0.0f,
                                             Theme::shellDark, 0.0f, static_cast<float> (getHeight()),
                                             false));
    g.fillAll();
    ...
    g.setColour (Theme::bezelHi.withAlpha (0.6f));
    ...
    paintScreenSurround (g, spectrogramView.getBounds(), spectrogramLabelArea,
                         "SPECTRAL DENSITY",
                         nyquist > 0.0 ? "LINEAR / 0-" + juce::String (nyquist / 1000.0, 1) + " KHZ"
                                       : juce::String ("LINEAR"));
