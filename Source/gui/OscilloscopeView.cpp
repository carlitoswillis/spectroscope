...
void OscilloscopeView::mouseMove (const juce::MouseEvent& event)
{
    cursor = event.getPosition();
    repaint();
}

void OscilloscopeView::mouseExit (const juce::MouseEvent&)
{
    cursor = { -1, -1 };
    repaint();
}
...
void OscilloscopeView::drawCursorReadout (juce::Graphics& g)
{
    const auto bounds = getLocalBounds().toFloat();
    ...
    if (sampleRate <= 0.0 || bounds.getWidth() <= 0.0f)
        return;
    ...
    // Same sweep length the SWEEP readout labels, scaled by fraction across.
    const auto sweepMs = 1000.0 * windowLength / sampleRate;
    const auto ms = (fx - bounds.getX()) / bounds.getWidth() * sweepMs;
